...
from vxquant.model.broker import vxTick, vxBar
...
def _quote_convter(cnt, volunit):
    return {
        f"bid{cnt}_v": lambda x: x[f"bid_vol{cnt}"] * volunit,
        f"bid{cnt}_p": lambda x: x[f"bid{cnt}"] / 100 * volunit,
        f"ask{cnt}_v": lambda x: x[f"ask_vol{cnt}"] * volunit,
        f"ask{cnt}_p": lambda x: x[f"ask{cnt}"] / 100 * volunit,
    }
...
tdxConBondTickConvter.add_convertors("lasttrade", lambda x: round(x["price"] / 100, 4))
...
for i in range(1, 6):
    for k, v in _quote_convter(i, 10).items():
...
tdxETFLOFTickConvter.add_convertors("lasttrade", lambda x: round(x["price"] / 10, 4))
tdxETFLOFTickConvter.add_convertors("volume", lambda x: x["vol"] * 10)
tdxETFLOFTickConvter.add_convertors(
    "created_dt", lambda x: tdx_to_timestamp(x["reversed_bytes0"])
)
