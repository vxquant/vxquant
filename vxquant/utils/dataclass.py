...
class vxField:
    ...
    def __set_name__(self, owner, name):
        ...
        self._fget = lambda obj, owner: getattr(obj, f"_{name}", self.default)
        ...
        if name == "updated_dt":
            ...
            def fset(obj, value):
                setattr(obj, f"_{name}", self._convertor_factory(value))
                obj.updated_dt = vxtime.now()
...
class vxUUIDField(vxField):
    ...
    def __init__(self, auto=True):
        ...
        super().__init__(default_factory=lambda: str(uuid.uuid4()) if auto else "")
...
class vxDataClass(metaclass=vxDataMeta):
    ...
    def __str__(self):
        ...
        try:
            ...
            return f"< {self.__class__.__name__}(id-{id(self)}) : {self.message} >"
        else:
...
def _(obj):
    return json.dumps(obj.message, ensure_ascii=False, indent=4)


class vxDataConvertor:
    """数据转换器基础类"""

    def __init__(self, target_cls, rename_dict: Mapping = None) -> None:
    ...
    def __call__(self, other_data, key="", **kwargs):
        ...
        def _convert_col(col):
            try:
                ...
                logger.error(
                    f"target class: {self._target_cls.__name__} Other_data load col:{col} err. {err}",
                    exc_info=True,
                )
                logger.debug(f"other_data={other_data}")
            return col, self._defaults_data.get(col, None)

        data.update(map(_convert_col, self._target_cls.__slots__))
        data.update(**kwargs)

        instance = self._target_cls(**data)
        return instance[key], instance if key in instance.keys() else instance


def _to_vxdict(obj: Any):
    if isinstance(obj, vxDict):
        return obj
    ...
    if isinstance(obj, abc.Sequence):
        return [_to_vxdict(e) for e in obj]

    return obj


class vxDict(abc.Mapping):
    """定制的dict类型"""
    ...
    def __eq__(self, __o: abc.Mapping) -> bool:
        if len(self) != len(__o):
            return False
    ...
    def __str__(self):
        try:
            return f"< {self.__class__.__name__}(id-{id(self)}) : {to_json(self.__dict__)} >"
